...
class ListPoint extends Component {
    ...
    async componentWillMount() {
        ...
        var that = this;
        // console.log(domain + '/api/user_v1/api_money_point_history.php?token=' + token + '&user_id=' + admId + '&type=1&page=' + page + '&limit=' + limit);

        fetch(domain + '/api/user_v1/api_money_point_history.php?token=' + token + '&user_id=' + admId + '&type=1&page=' + page + '&limit=' + limit, {
            headers: {
                'Cache-Control': cache
            }
        })
    ...
    renderListPro() {
        let html = [];
        let listPoint = this.state.listPoint;
        let listPointDown = this.state.listPointDown;
        ...
        for (let i = 0; i < listPoint.length; i++) {
            ...
            html.push(
                <View key={i} style={{ paddingBottom: 10, marginHorizontal: 20, marginBottom: 10, borderBottomWidth: 1, flexDirection: 'column', justifyContent: 'flex-start', alignItems: 'flex-start', }}>
                    <Text style={{ flex: 1 }}>Mã vé: <Text>{listPoint[i].ve_id + listPoint[i].ve_hid}</Text></Text>
                ...
                <ScrollView>
                    ...
                    {!this.state.loading &&
                        <View style={{ flexDirection: 'column' }}>
                            ...
                            <Text style={{ fontSize: 20, fontWeight: 'bold', marginLeft: 20, marginBottom: 10 }}>LỊCH SỬ TÍCH ĐIỂM</Text>
